            ...
            <div className="relative bg-white border border-slate-200 rounded-[1.5rem] p-6 shadow-sm hover:shadow-md hover:border-primary/40 transition-all duration-300 group overflow-hidden h-full flex flex-col gap-4">
              ...
              <div className="z-10 mt-2">
                <div className="flex items-start justify-between mb-1">
                  <h3 className="font-bold text-slate-900 text-xl">{cred.metadata?.name || `Credential #${cred.tokenId}`}</h3>
                  {cred.uri && (
                    <a
                      href={resolveIpfs(cred.uri)}
                      target="_blank"
                      rel="noreferrer"
                      className="text-xs font-semibold flex items-center gap-1 text-primary hover:text-primary-hover transition-colors border border-primary/20 hover:border-primary/50 rounded-full px-2.5 py-1 bg-primary/5"
                    >
                ...
                <p className="text-sm text-slate-600 font-medium leading-relaxed mb-4">{cred.metadata?.description || "Verifiable Proof of Work"}</p>

        ...
        {isImageUrl(previewUrl) ? (
          ...
          <div className="flex flex-col space-y-3 w-full">
            ...
            <iframe
              ...
              loading="lazy"
            />
            {/* Escape hatch for blocked iframes */}
            <div className="flex justify-between items-center bg-white/[0.02] px-4 py-2.5 rounded-lg border border-white/10">
              <span className="text-xs text-white/40">
                Is the preview grayed out or blocked?
              </span>
              <a
                href={previewUrl.startsWith("http") ? previewUrl : `https://${previewUrl}`}
                target="_blank"
                rel="noopener noreferrer"
                className="text-xs px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white/70 hover:text-white rounded-md transition-all flex items-center gap-1.5"
              >
                Open in New Tab ↗
              </a>
            </div>
